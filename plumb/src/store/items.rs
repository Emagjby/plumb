...
pub fn load_items(root: &Path) -> Result<Vec<Item>, StoreError> {
    ...
    let scb = std::fs::read(&items_file)
        .map_err(|e| StoreError::ReadError(format!("Failed to read items file: {e}")))?;

    let decoded = decode(&scb)
        .map_err(|e| StoreError::ReadError(format!("Failed to decode SCB: {:?}", e)))?;
    ...
    let items: Result<Vec<Item>, _> = items_list
        ...
        .map(|value| {
            ...
            let id = match map.get("id") {
                Some(Value::Int(i)) => usize::try_from(*i).map_err(|e| {
                    StoreError::ReadError(format!("Invalid 'id' value (not a valid usize): {e}"))
                })?,
                _ => {
                    return Err(StoreError::ReadError(
